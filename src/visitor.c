...
AST_T* visitor_visit_function_call(visitor_T* visitor, AST_T* node)
{
	...
	for (int i = 0; i < (int) node->function_call_arguments_size; i++)
	{
		...
		// create a new variable definition with the value of the argument in the function call.
		AST_T* ast_vardef = init_ast(AST_VARIABLE_DEFINITION);
		ast_vardef->variable_definition_value = ast_value;

		// copy the name from the function definition argument into the new variable definition
